 ...
 * src/dms/packages/dms/src/patterns/page/components/sections/components/dataWrapper/utils/timeFilter.js
 *
 ...
 */
const requiredTickGranularity = (value) => {
 ...
 */
const walkTreeForTickGranularity = (node) => {
    if (!node) return null;
 ...
 */
const parseTimeFilterURL = (token) => {
 ...
 * are exposed.
 */
const isAxisExposed = (value, axis) => {
 ...
 */
const serializeTimeFilterURL = (value) => {
 ...
 * for `op === 'time'` leaves.
 */
const mergeUrlOntoExposedAxes = (persisted, parsed) => {
...
module.exports = {
  ...
  DAY_NAMES,
  DAY_INDEX,
  TICK_GRANULARITY_BY_UNIT,
  TICK_RANK,
  finerOf,
  requiredTickGranularity,
  walkTreeForTickGranularity,
  formatRangeEntry,
  formatDow,
  humanLabel,
  HHMM,
  ...
  serializeTimeOfDay,
  isAxisExposed,
  serializeTimeFilterURL,
  mergeUrlOntoExposedAxes,
};
