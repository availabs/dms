...
export function SectionsChipView({ value, row = {}, className = '' }) {
    ...
    return (
        <span
            className={`inline-flex items-center gap-1 border border-[var(--t-rule)] bg-[var(--t-panel)] rounded-full px-2 py-0.5 text-[11px] font-semibold text-[var(--t-graphite)] cursor-pointer select-none hover:border-amber-400 hover:bg-amber-50 transition-colors ${className}`}
            title={`${count} sections${staleCount ? ` — ${staleCount} on outdated views` : ''}`}
        >
            <span className="text-[8px] text-[var(--t-pencil)]">▶</span>
            {count}
            {staleCount > 0 && (
