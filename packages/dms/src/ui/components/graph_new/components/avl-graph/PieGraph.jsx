...
export const DefaultPieHoverComp = props => (
  // No `showTotals` default, deliberately — see DefaultBarHoverComp. Pie also orders rows by
  // value descending where Bar reverses the key array.
  <SeriesRowsHoverComp { ...props }
    orderedKeys={ (keys, data) => keys.filter(k => get(data, ["data", k], 0))
                    .sort((a, b) => get(data, ["data", b], 0) - get(data, ["data", a], 0)) }
    colorForKey={ (data, key) => get(data, ["colorMap", data.index, key], null) }/>
);

const DefaultHoverCompData = {
  HoverComp: DefaultPieHoverComp,
