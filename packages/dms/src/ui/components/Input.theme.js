// Flat by default. A site theme MAY promote this to `{options, styles:[…]}` —
// Input resolves it with getComponentTheme(theme,'input',activeStyle), and the
// callers that already pass a style name down (filter controls via
// `theme.filters.<style>.controlStyle`, column types) then restyle the box the
// same way a named `multiselect` style restyles a select trigger. Flat stays
// flat: getComponentTheme returns this object as-is and activeStyle is ignored.
