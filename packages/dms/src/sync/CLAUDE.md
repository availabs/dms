...
Binds a page's `draft_sections` to a shared `Y.Array` in a per-page collab room (keyed by the page id, same relay as section rich-text rooms). `sectionArray.jsx`'s add/edit/remove/move apply ops to it and send its merged contents as `draft_sections`. **Once the room has content it wins over the database** — it only seeds from `draft_sections` while never-written (`knownEmpty`), and the server persists it indefinitely (`yjs_states`). So any writer that changes `draft_sections` outside the room leaves it stale, and the next browser section save reverts the page to it (Bug 20 in `planning/tasks/current/concurrent-page-editing-data-loss.md`). The CLI syncs the room after its writes (`cli/src/utils/room-sync.js`, `dms page sync-room`); Discard, the section-groups/settings panes, template apply, raw-SQL scripts, and every save from a sync-OFF build of the same site do **not** yet. Exposes `onStructureRoomsChange`/`getStructureRooms`/`replaceStructureRoomContents` for `room-health.js`. Each room tracks `lastLocalAt` (this tab's own last op) and `lastRemoteOpAt` (last real peer op) — distinct from `lastRemoteAt`, which is also stamped at connect time for `waitForQuiet`.

### `room-health.js`
Detects a stale page-structure room and repairs it, surfaced in the user menu. `checkRoomHealth(fetchSaved)` compares the open page room with the page's saved `draft_sections`, read fresh from the server by the caller (userMenu passes `api/index.js`'s `loadItemFresh`, which invalidates Falcor's cache for that one item and bypasses the local IndexedDB mirror — the mirror can lag the server). Reports `stale` only if the difference survives a re-check 2 s later with no pending pushes and no room op in the last 3 s; otherwise `busy`, auto-retried up to 5× (4 s apart). `repairRoom(fetchSaved)` re-reads the saved row, replaces the room's array, then calls `sync-manager.js`'s `revalidateNow()` so the tab's own view catches up. States: `idle` · `checking` · `ok` · `busy` · `stale` · `repairing` · `error`; `peerActive` flags a peer op in the last 10 s (the repair label warns). Lazily imported by userMenu only when `VITE_DMS_SYNC=1` (its own chunk).
...
### Status UI
No dedicated component in this module — the connection status is surfaced through the page pattern's user menu instead: `patterns/page/components/userMenu.jsx` subscribes to `onStatusChange`/`getPendingCount`/`onCollabChange`/`getCollabInfo` (dynamic `import('../../../sync/sync-manager.js')`, gated on `VITE_DMS_SYNC`) and renders a status ring around the avatar plus a status row (dot, label, pending count, collab peer count) inside the user dropdown. Ring/dot colors per status live in `userMenu.theme.jsx` (`syncRing*`/`syncDot*` keys). The same menu also runs `room-health.js`'s check once when an edit page's room becomes ready and again when the tab becomes visible (≥30 s apart); a stale room turns the ring red (`syncRingStale`, outranks connection status) and adds a status row with **Update room from database** / **Check page room again** (`syncRoom*` keys).
