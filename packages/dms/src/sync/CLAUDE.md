...
### Status UI
No dedicated component in this module — the connection status is surfaced through the page pattern's user menu instead: `patterns/page/components/userMenu.jsx` subscribes to `onStatusChange`/`getPendingCount`/`onCollabChange`/`getCollabInfo` (dynamic `import('../../../sync/sync-manager.js')`, gated on `VITE_DMS_SYNC`) and renders a status ring around the avatar plus a status row (dot, label, pending count, collab peer count) inside the user dropdown. Ring/dot colors per status live in `userMenu.theme.jsx` (`syncRing*`/`syncDot*` keys). The same menu also runs `room-health.js`'s check once when an edit page's room becomes ready and again when the tab becomes visible (≥30 s apart); a stale room turns the ring red (`syncRingStale`, outranks connection status) and adds a status row with **Update room from saved** / **Check page room again** (`syncRoom*` keys).
