...
const HoverComp = ({ data, layer }) => {
  ...
  const getAttributes = React.useMemo(() => (
    typeof attributes?.[0] === 'string'
      ? attributes
      : attributes.map(d => d.name || d.column_name)
  ).filter(d => !['wkb_geometry'].includes(d)), [attributes]);
  const getAttributesKey = React.useMemo(() => getAttributes.join('|'), [getAttributes]);

  React.useEffect(() => {
    if (!id || !view_id || !getAttributes.length) return;
  ...
  }, [falcor, pgEnv, view_id, id, getAttributesKey]);
