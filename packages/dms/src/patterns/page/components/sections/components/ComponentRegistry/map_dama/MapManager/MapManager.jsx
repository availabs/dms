...
function SymbologyRow ({tabIndex, row, rowIndex}) {
  ...
  const visible = state?.symbologies?.[symbology.id]?.isVisible

  const toggleVisibility = useMemo(() => {
    return () => setState(draft => {
      draft.symbologies[symbology.id].isVisible  = !draft.symbologies[symbology.id].isVisible
      Object.keys(draft.symbologies[symbology.id].symbology.layers).forEach(layerId => {
        const curLayer = draft.symbologies[symbology.id].symbology.layers[layerId];
        curLayer.layers.forEach((d,i) => {
          let val = get(state, `symbologies[${symbology.id}].symbology.layers[${layerId}].layers[${i}].layout.visibility`,'') 
          let update = val === 'visible' ? 'none' : 'visible'
          draft.symbologies[symbology.id].symbology.layers[layerId].layers[i].layout =  { "visibility": update }
        })
      })
    })
  }, [state, setState]);
                ...
                <Menu.Item >
                  {({ active }) => (
                    <div 
                      ...
                      onClick={() => {
                        setState(draft => {
                          delete draft.symbologies[symbology.id]
