...
function SectionGroupControl({
  ...
  const currentStyle = getComponentTheme(
    themeFromContext,
    "pages.sectionGroupsPane",
  );
  ...
  if (isSection) {
    return (
      <div
        className={`${currentStyle.sectionTargetWrapper} ${["content", "bottom"].includes(group.id) ? currentStyle.sectionTargetDivider : ``}`}
      >
        <span>{group?.title || group?.id}</span>
        <button onClick={onAdd} className={currentStyle.addGroupBtn}>
          + Add Group
        </button>
      </div>
    );
  ...
  const menuConfig = [
    {
      ...
      items: ["off", "show"].map((name) => ({
        icon:
          name === (fullGroupData?.full_width || "off")
            ? "CircleCheck"
            : "Blank",
        name: name,
    ...
    },
    {
      ...
      showValue: true,
      items: (theme?.layoutGroup?.styles || [])
        .map((d) => d.name)
        .map((name) => ({
          icon:
            name === (fullGroupData?.theme || "default")
              ? "CircleCheck"
              : "Blank",
          name: name,
  ...
  return (
    <div
      className={`${currentStyle.sectionGroupWrapper}
    ...
    >
      <div className={currentStyle.titleWrapper}>
        <Icon
          ...
          className={
            group.isGroup
              ? currentStyle.sectionGroupIcon
              : currentStyle.pageSectionIcon
          }
        />
        <span
          className={
            group.isGroup
              ? currentStyle.sectionGroupTitle
              : currentStyle.pageSectionTitle
          }
        >
      ...
      <div className={currentStyle.controlsWrapper}>
        ...
        {fullGroupData.isGroup ? (
          <Icon
            icon={"CaretDown"}
            className={currentStyle.expandGroupIcon}
...
export default function SectionGroupsPane() {
  ...
  const deleteGroup = (groupName) => {
    const newItem = {
      id: item.id,
      draft_section_groups: cloneDeep(item?.draft_section_groups || []).filter(
        (d) => d.name !== groupName,
      ),
    };
    apiUpdate({ data: newItem });
      ...
      result[target] = {
        ...
        children: childGroups.map((g) => ({
          ...
          children: (item?.draft_sections || [])
            .filter(
              (s) => s.group === g.name || (!s.group && g.name === "default"),
            )
            .map((s) => ({ id: s.id, type: s?.element?.["element-type"] })),
        })),
      };
    });
          ...
          return (
            <SectionGroupControl
              ...
              onUpdateAttribute={(attr, value) =>
                updateGroupAttribute(group.id, attr, value)
              }
            />
          );
        }}
...
const togglePageSetting = async (item, type, value = "", apiUpdate) => {
  ...
  if (
    type === "header" &&
    !item?.[sectionType]?.filter((d) => d.is_header)?.[0]
  ) {
