...
- [Full-text search over several columns](./full-text-search-filter.md) — a single keyword search box that matches across many columns at once, with no custom component: a `Filter` control with `operation:'like'` (renders a text box) writing `searchParamKey:'search'`, plus an **`OR` group of `like` leaves — one per searchable column** — on each data section. Covers the exact config, why it works (`like`→`%v%`; an empty leaf drops out so an empty box = no constraint), the multi-column-facet variant (same shape, `op:'filter'`), the client-side `localFilter` alternative, and the case-sensitivity (`ILIKE`) + large-table performance caveats (`pg_trgm`, debounce). Worked example: the MNY `actions_index` page (16-column search over ~17.8k rows).
