...
| The page 500s and renders **"You do not have permission to view this page"** right after you added a section | **`section create` was run without `--pattern`.** `resolvePagePattern` then picks the app's **first** pattern and writes the row as `<app>+<that-pattern>\|component`. The row is created *and* attached to `draft_sections`, so nothing errors at create time — the page just dies on load. Reads like an auth failure and isn't. | `dms section delete <id> --pattern <right-one> --page <page>`, then re-create **with `--pattern`**. Always pass it on `section create` and `section list`. Note `section update` / `section delete`… `update` takes a row id and **rejects** `--pattern`; `delete` accepts it. |
| A section renders its **page-level** data (title, published pill) but none of its **authored** data (labels, copy, display settings) | The section mounts through `dataWrapper`, which seeds a fresh default state when `element-data` lacks the `filters` / `columns` / `data` / `externalSource` scaffolding — and takes `display` down with it. This bites components that bind no source at all, where the scaffolding looks like dead weight. | Include it anyway: `{"filters":{"op":"AND","groups":[]},"display":{…},"columns":[],"data":[],"externalSource":{"columns":[]}}`. Copy the shape from a working instance of the same `element-type` (`dms section dump <id>`). |
| A "create it if it isn't there" script makes a **duplicate** every run | The idempotency check looked for `data.element['element-type']`. `dms section list` returns a **summary** row that flattens it onto `data['element-type']` — the nested path is only present on `section dump`. | Check both paths, or key the check off `section dump` for each id. |
| `/edit/<slug>` shows a **different pattern's page** (or your page is unreachable) | **Multiple patterns share `base_url: /`** on the site. A subdomain-specific pattern shadows a `subdomain: null` pattern at `/` (this used to be `npmrds_sub` on the `npmrds` subdomain — as of 2026-09-02 it's a `www`-subdomain, `/npmrds` path-mount instead, so it no longer illustrates this exact shadow; find a currently-subdomained pattern via `dms site tree` if you need a live example). | A `subdomain: null` pattern is the **default/fallback** — reach it on the **bare host**: `http://localhost:5173/edit/<slug>` (mint a token for *that* origin: `mint-token.mjs --origin http://localhost:5173`). Confirm ownership with `dms site tree` (lists every pattern's `base_url`/`subdomain`). Also pass `--pattern <id>` on `section create` so type resolution doesn't pick a sibling `/` pattern. |
