...
- **Writing a `lexical` (Rich Text) section's `element-data.text` as a plain string from a script/CLI.** The read-only `RichtextView` checks `text?.root` directly and renders nothing for a bare string — only the *edit* component (`ui/components/lexical/index.jsx`'s `parseValue`/`textToLexicalJSON`) auto-upgrades plain text on load. A programmatic writer must build the tree itself: `{root:{children:[{type:"paragraph", children:[{type:"text", text:"...", detail:0, format:0, mode:"normal", style:"", version:1}], direction:"ltr", format:"", indent:0, version:1}], direction:"ltr", format:"", indent:0, version:1}}`. This looks fine in a draft (edit mode upgrades it) and renders **empty once published** — worked example: `report_build.mjs`'s `textToLexicalTree`/`lexicalTreeToText` (also handles multi-paragraph, which the editor's own single-paragraph helper doesn't).
