...
| **582** | **983** | NPMRDS_V6_tmc_meta | `clickhouse.npmrds_meta` / `s582_v983_NPMRDS_V6_tmc_meta` | Same TMC identification/attributes as 1946/3298 below (byte-identical 58-column schema), but spans one row per (tmc, year), 2016-2026, instead of being frozen at 2025. **Correction 2026-08-03: a 2017 row DOES exist** (36,181 rows, confirmed live `SELECT count() ... WHERE year=2017`) — the "no 2017 row" claim below was stale/wrong, caught while scoping the auto-generated-corridor-routes work in `planning/transportny/tasks/current/dynamic-reports-and-route-tags.md`. Also carries `tmclinear`/`road_order`/`county_code`/`region_code` (not listed in the original column summary below) — useful for corridor-grouping work, see that task file. **Correction 2026-08-12: the real live schema is 58 columns, confirmed via `DESCRIBE TABLE npmrds_meta.s582_v983_NPMRDS_V6_tmc_meta`** — a prior pass (this doc, and `vocabulary.json`'s own cached copy) undercounted it at 11, listing only the columns `DELAY_EXPR`/CO2 measures happened to reference at the time. The real table is a confirmed **strict superset** of 455/3464 below (every one of its ~43 columns, verbatim names) plus `wkb_geometry` and admin codes (`state_code`/`county_code`/`region_code`/`mpo_code`/`ua_code`/`state_name`/`county_name`/`ua_name`/`is_interstate`) 455/3464 never had. | **THE ONE CANONICAL TMC-METADATA JOIN as of 2026-08-12 — `META_JOIN` for every measure that needs TMC attributes, full stop.** Originally (round 59, 2026-07-17) replaced 1946/3298 for hoursOfDelay/avgHoursOfDelay/co2Emissions/avgCo2Emissions only, joined with a COMPOUND key (`tmc=tmc AND toYear(ds.date) as ... = year`, the same calculated-dsColumn mechanism `DIST_KEY_EXPR` below uses) so every fact row resolves against its OWN date's year instead of a single frozen snapshot. **Extended 2026-08-12 to `speed`/`speedTruck` (AVL Graph) and Info Box's `speed`/`length`/`aadt` and Route Compare's `speed`** — these used to read 455/3464 instead (see that row below, now removed as a live join). Root cause: 455/3464 is a single static snapshot with no year dimension, so any measure reading it got a frozen, potentially-wrong-vintage value for `miles`/`aadt`/etc. regardless of what year was actually being queried — confirmed real and non-trivial (not just theoretical staleness): ~96% of TMCs have `miles` that genuinely changes across years (real TMC-network-vintage step-changes, e.g. a confirmed 20% jump for one sampled TMC between 2018→2019 — not floating-point noise), and `aadt` drifts nearly every year for nearly every TMC (expected, it's an annual traffic count by design). Year coverage confirmed complete before the swap: 2016-2026, all 11 years, no gaps. The shared base template every fresh AVL Graph/Info Box/Route Compare section clones from (`tmc_travel_time_line_graph`, row `2187310`) was updated to carry `META_JOIN` as its own default too, so nothing can silently inherit 455/3464 again by omission. Full record: `planning/transportny/tasks/current/dynamic-reports-and-route-tags.md`'s "Metadata-unification arc" section. Quantified impact of the ORIGINAL (2026-07-17) 1946→582/983 swap on a real report year (2019 vs the old 2025 snapshot): 46.5% of TMCs have a different `aadt`, 31% a different `congestion_level` (which itself feeds `DIST_KEY_EXPR`'s AADT-distribution weighting), 146 TMCs missing entirely. ~~No 2017 row exists (gap-logged, not fixed)~~ **retracted 2026-08-03, see left column** — `DELAY_EXPR`/`CO2_EXPR_PASSENGER`/`CO2_EXPR_TRUCK` guard the join-miss case with `nullIf(table1.aadt/miles, 0)` regardless, so this doesn't change any live behavior, just the documented reason. |
| ~~1946~~ | ~~3298~~ | ny_2025_tmc_meta | `clickhouse.npmrds_meta` / `s1946_v3298_ny_2025_tmc_meta` | TMC identification/attributes: miles, avg_speedlimit, aadt, aadt_singl, aadt_combi, congestion_level, directionality, f_system, faciltype, avg_vehicle_occupancy, geometry, etc. | **Superseded by 582/983 above (round 59)** — kept here for history; no live template should reference this source anymore. `categories: [["Inactive","archive"]]` despite having been actively used — categorization was stale/not a reliability signal even before the swap. |
...
| ~~455~~ | ~~3464~~ | NPMRDS TMC Identification V5/V6 | `clickhouse.npmrds_raw_tmc_identification` / `s455_v3464_NPMRDS_TMC_Identification_V5_V6` | Per-TMC static attributes (44 cols): `tmc, road, direction, intersection, miles, f_system, faciltype, structype, thrulanes, route_numb, aadt, aadt_singl, aadt_combi, nhs, nhs_pct, truck, wkb_geometry` (GeoJSON MultiLineString despite the name), etc. | **REMOVED as a live join, 2026-08-12 — superseded by 582/983 (`META_JOIN`) above, no live template should reference this source anymore.** Used to be *the default join every `avl_graph_template` row carried*, inherited automatically whenever a template row is deep-copied from `tmc_travel_time_line_graph`'s live `base_state` (round-38 "carry the join forward" fact) — that base template row is now on `META_JOIN` instead. Confirmed (2026-08-12) that 582/983 is a strict superset of every column here (verbatim names) plus more, and lacks this source's one real advantage — a fixed, never-changing value — which turned out to be exactly the problem, not a feature (see 582/983's row above). This source's own `TMC_IDENTIFICATION_JOIN` constant was deleted from `vocabulary.json` entirely; kept as a row here for history/citation only. `fetchTmcMiles.js` (RRL's route-length display, the one non-report-query consumer) was migrated to `META_JOIN` too. |
...
**Corrected 2026-08-12**: `speed`/`travelTime`/`length`/`aadt` used to read 455/3464
(TMC Identification, a frozen single-snapshot join) instead of 582/983 — that row is struck
through below; the table now reflects the current, correct state. See 582/983's row above and
`planning/transportny/tasks/current/dynamic-reports-and-route-tags.md`'s "Metadata-unification
arc" for the full investigation and fix.

| Measure family | Source(s) used | Why | Constant(s) to edit for a swap |
|---|---|---|---|
| speed, speedTruck (every graph type + Route Compare + Bar Graph Summary + Info Box `speed`) | 583 (fact) + **582/983** (`META_JOIN`, year-matched — corrected 2026-08-12, was 455/3464) | `SPEED_EXPR` needs `table1.miles` for the two-level per-TMC→route composition (round 34/35 backport); `miles` must be year-correct since it genuinely varies by TMC-network vintage (confirmed live, ~96% of TMCs) | `vocabulary.json`'s `measures.speed`/`speedTruck.requiresJoin` (now `["META_JOIN"]`), plus the shared base template row (`tmc_travel_time_line_graph`, `2187310`) if `miles` itself moves again |
| travelTime (every graph type + Route Compare + Bar Graph Summary + Info Box `travelTime`/`avgTT-byDateRange`) | 583 (fact) only — joinless | `TRAVEL_TIME_EXPR` never references `table1` at all; still needs SOME join present so the base table gets aliased `ds` (dms-server quirk), which is why it still carries `META_JOIN` (the base template's own default) even though it doesn't read from it | `TRAVEL_TIME_EXPR` (`scripts/npmrds-reports/convert_old_reports.py:389-408`) |
| hoursOfDelay, avgHoursOfDelay | 583 + 582/983 (`META_JOIN`, year-matched — round 59, was 1946/3298) + 2056/3524 (aadt_distributions) | `DELAY_EXPR` needs `avg_speedlimit`/`faciltype` (delay threshold) from 582, plus epoch-level AADT weighting from 2056 | `META_JOIN`, `AADT_DIST_JOIN`, `DELAY_EXPR`, `DIST_KEY_EXPR` (`scripts/npmrds-reports/convert_old_reports.py`) |
...
| length, aadt (Info Box TMC-attribute measures) | 583 (WHERE-scoping only) + **582/983** (`META_JOIN` — corrected 2026-08-12, was 455/3464) | `LENGTH_EXPR`/`AADT_EXPR`/their `_TMC_EXPR` variants read `table1.miles`/`table1.aadt`, now year-correct | `LENGTH_EXPR`, `LENGTH_TMC_EXPR`, `AADT_EXPR`, `AADT_TMC_EXPR` (`:232-254`), `_ensure_static_info_box_template`'s explicit `join` (`info_box_templates.py`) |
| ~~length, aadt (old, pre-2026-08-12)~~ | ~~583 (WHERE-scoping only) + 455/3464 (default join, unmodified)~~ | ~~read straight off whatever the base template's default join happened to be~~ | superseded — see the row above |
...
**Deliberately excluded from combination — Info Box multi-measure (2026-08-12)**: `reliability`
(the 1410 PM3 row above) can't be combined with any other Info Box measure in one box — it's the
only Postgres/`pgFederated` join in the catalog, a fundamentally different query mechanism from
every other measure's plain ClickHouse `join.sources.tableN`, and combining the two has never
been verified. See `check_info_box_measure_combo()`/`INFO_BOX_MULTI_JOIN_GROUP`
(`info_box_templates.py`) for the enforcement.

**Not currently used by any live template** (bank only, documented below): 1722, 2001,
...
| 582 | NPMRDS TMC Network Shapefile - Enhanced V6 | Partially duplicated across ClickHouse + Postgres, built on the RITIS TMC-ID shapefile. View 983 (`clickhouse.npmrds_meta.s582_v983_NPMRDS_V6_tmc_meta`, see registered table above) is now `META_JOIN`'s live source (round 59); the other views under this source_id are the per-year geometry tile views (see the table below). Cross-referenced from 583's `metadata.npmrds_tmc_meta_source_id: 582`. |
