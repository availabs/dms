...
export function ConfirmInput ({
  ...
  EditIcon:EditIconFromProp, ConfirmIcon:ConfirmIconFromProp, CancelIcon:CancelIconFromProp
}) {
  const { theme = { input: inputTheme } } = React.useContext(ThemeContext) || {}
  ...
  const EditIcon = typeof EditIconFromProp === 'string' ? <Icon icon={EditIconFromProp} /> :
      EditIconFromProp && typeof EditIconFromProp === 'object' ? <EditIconFromProp /> :
      <Icon icon={'PencilIcon'} className={'size-5.5'}/>;

  const ConfirmIcon = typeof ConfirmIconFromProp === 'string' ? <Icon icon={ConfirmIconFromProp} /> :
      ConfirmIconFromProp && typeof ConfirmIconFromProp === 'object' ? <ConfirmIconFromProp /> :
      <Icon icon={'CircleCheck'} />;

  const CancelIcon = typeof CancelIconFromProp === 'string' ? <Icon icon={CancelIconFromProp} /> :
      CancelIconFromProp && typeof CancelIconFromProp === 'object' ? <CancelIconFromProp /> :
      <Icon icon={'CircleX'} />;
  return (
    <span className={`${theme?.input?.inputContainer}`}>
      { editing ? 
      ...
      {!editing ? 
        (<div className={`${theme?.input?.confirmButtonContainer} ${theme?.input?.editButton}`} onClick={() => setEditing(true)}>{EditIcon}</div>) :
