...
export default function ButtonComp ({ children, disabled, onClick=()=>{}, type='default', padding='p-2', rounded, className, ...props}) {
  const { theme = { button: buttonTheme } } = React.useContext(CMSContext) || {}
  return (
    <Button disabled={disabled} className={`${className} ${theme?.button?.[type] || theme?.button?.default} ${padding} ${rounded || theme?.button?.rounded}` } onClick={onClick} {...props}>
      {children}
    </Button>
  )
}
