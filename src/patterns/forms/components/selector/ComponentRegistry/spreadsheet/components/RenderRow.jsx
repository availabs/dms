...
export const RenderRow = ({
    ...
    <div key={`data-${i}`}
         className={`${d.totalRow ? `sticky bottom-0 z-[1]` : ``} grid ${allowEdit ? c[visibleAttributes.length + 3] : c[visibleAttributes.length + 2]} 
                                    divide-x divide-y ${isDragging ? `select-none` : ``} ${striped ? `odd:bg-gray-50` : ``} ${d.totalRow ? `bg-gray-100` : ``}`}
    ...
    >
        <div key={'#'}
             ...
             onMouseDown={e => handleMouseDown({e, index: i, setSelection, setIsDragging, startCellCol, startCellRow, selection})}
             onMouseMove={e => handleMouseMove({e, index: i, isDragging, startCellCol, startCellRow, setSelection})}
        ...
        {visibleAttributes
            ...
            .map((attribute, attrI) =>
                <RenderCell
                    ...
                    i={i}
                    item={d}
                    onMouseDown={e => handleMouseDown({e, index: i, attrI, setSelection, setIsDragging, startCellCol, startCellRow, selection})}
                    onMouseMove={e => handleMouseMove({e, index: i, attrI, isDragging, startCellCol, startCellRow, setSelection})}
                    onMouseUp={e => handleMouseUp({setIsDragging})}
                    onClick={() => {
