...
export default (props) => {
    ...
    return (
        <div>
            ...
            <Button disabled={credentials.password !== credentials.verifyPassword}
                onClick={async () => {
                console.log('call signup', credentials, AUTH_HOST)
                await callAuthServer(`${AUTH_HOST}/password/set`, {password: credentials.password, token, project: PROJECT_NAME})
                    .then(res => {
                        console.log('res', res)
                        if (res.error) {
                            setStatus(res.error)
