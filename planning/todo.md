...
- [x] [Card `allowDelete` + themeable form action row](./tasks/completed/card-allow-delete-and-form-action-row.md) — DONE 2026-09-12. The dataWrapper/API/server already deleted rows; the Card had no button. Now `display.allowDelete` renders a two-step Delete on each record (with `closeModalOnDelete` + a `delete_publish` provider), and save/cancel/add/delete take theme classes; the action-row wrappers span the card so right-aligned means the card's edge.
- [x] [Section menu resolves Width/Row span/Border/Padding/Shadow from the band's sectionArray style](./tasks/completed/section-menu-uses-band-style.md) — DONE 2026-09-12. The menu resolved `pages.sectionArray` without the band's style name (seven lookups), so a theme whose named styles replace `sizes` (WCDB's 12-column `GRID_12`) offered the default fractions, which matched nothing at render and fell back to full width. Now `sectionArray.jsx` passes `group` down, `section.jsx` hands `sectionArrayStyle` to the menu, and one `sa` lookup feeds all seven controls.
...
- [x] Playlist detections tagged with the show that was on air — `show_id` on the now_playing schema (so DDL, `metadata.columns` and the INSERT all follow from one list), a `showResolver.js` that owns the three conversions this has gone wrong on before (station-local `AT TIME ZONE`, `ISODOW - 1` for the 0=Monday day numbering, `end='00:00'` read as 24:00), and ingest-time resolution in the webhook so history is not rewritten when a schedule is published. Which schedule a stream follows lives in `source.metadata.schedule` as a VIEW id. Overlapping airings resolve to the narrower slot. Backfilled 21,308 of 37,041 existing detections (`scripts/wcdb-admin/tag-playlist-with-shows.mjs`).
