...
- [ ] [dataWrapper blank-row fallback](./tasks/current/datawrapper-blank-row-fallback.md) — opt-in: when a data-driven section's query returns 0 rows AND the section has `display.useBlankRowFallback: true` set, `getData.js` synthesizes a single row keyed `column.normalName || column.name` from each column's optional `blankDefault` (any column type's value shape — text, hue, URL, etc.) and returns `{ length: 1, data: [{...defaults, _isBlankFallback: true}] }`. Card chrome renders normally; renderers that want to differentiate can read the marker. Per-column "Empty Default" toolbar entry mounts the column type's existing `EditComp` so authoring works for any type with no special-cased UI. **BC: defaults to false — no synthesis runs unless explicitly enabled, existing sections render byte-identical.** Calc columns just store the literal final value (no SQL re-evaluation). Driving use case: WCDB now-airing card during off-air hours. Error-fallback (network failures) deferred to a sibling task.
...
- [ ] [Card per-column padding overrides](./tasks/current/card-cell-padding-overrides.md) — extend the existing `cellPaddingBottom` knob to `cellPadding` (all sides) + `cellPaddingTop` / `cellPaddingLeft` / `cellPaddingRight`. Inline padding overrides on the cell wrapper, side-specific wins over `cellPadding`, `cellPadding` wins over ambient `cellsPadding`. Toolbar inputs added next to the existing Padding Below. Driving case: WCDB now-playing card title cell needs `cellPaddingTop: 0, cellPaddingBottom: 0` so the three-line text stack fits inside the album art's 96px height.
