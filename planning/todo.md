...
- [ ] [Replace wa-sqlite with direct IndexedDB](./tasks/current/sync-replace-sqlite-with-indexeddb.md) — sync already persists to IndexedDB via wa-sqlite's `IDBBatchAtomicVFS`; this drops the 2.2MB WASM SQL-engine layer on top of it. Audited every live SQL statement in `sync-manager.js` — 100% exact-key or `type LIKE 'x|%'` prefix scans, a clean fit for IndexedDB compound indexes; `use-query.js`'s arbitrary-SQL hook has zero call sites. Blocked on the task above.
