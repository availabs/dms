...
- [ ] [WCDB schedule migration + now-airing card](./tasks/current/wcdb-schedule-now-playing-card.md) — first real consumer of the `op:'time'` Phase 4 `instant` + `compareEnd` primitive. Two phases: (1) reshape the WCDB schedule view so its start/end columns are typed `timestamp`/`timestamptz` (calculated-column bridge over the existing `day_of_week` + text-time columns vs. full storage migration — pick before implementing), (2) configure a Card section on WCDB's home page bound to the schedule view with a time-filter leaf `{kind:'instant', at:'now'}` + `compareEnd: 'end_at'`, page size 1. No new component — reuses existing `Card`. `useNowTick` handles boundary refetch automatically. Includes a `documentation/schedule-card-recipe.md` write-up so other patterns (event calendars, "currently active" indicators) can copy the pattern.
