...
- [ ] [Page-editing data loss and blanking under sync](./tasks/current/concurrent-page-editing-data-loss.md) — live stress testing found **four** confirmed, reproducible bugs. Three under concurrency, still open: (1) concurrent section creation loses sections — `draft_sections` is a plain array mutated via client-computed-full-array + server shallow-JSON-merge with no concurrency control, so N simultaneous adds leave only 1 surviving, the rest orphaned in the DB (losing clients' own screens evict their own unsaved work live via WS, no reload needed) — a fully fleshed-out `Y.Array`-CRDT fix design is written up, not yet implemented; (2) concurrent same-position rich-text typing drops real characters (confirmed via multiset diff), correlated with a thrown `Invalid access: Add Yjs type to a document before reading data` error; (3) delete-vs-add race inferred (shared code path) but not independently reproduced. **(4) — found on the user's real dev site, needed no concurrency at all — ROOT-CAUSED AND FIXED**: adding/deleting any section permanently blanked the whole page (only a hard reload recovered it) the first time a page pattern's reads ever switched from Falcor to local-first sync in a session. Root cause, confirmed by a live `getItem('id')` vs `getItem(id)` A/B test against the real site: `api/index.js`'s ref-resolution coerced lookup ids to `Number`, but every server-synced IndexedDB row is string-keyed (server JSON always sends ids as strings) — so every ref lookup silently missed, leaving sections unresolved and unrendered. Fixed at both call sites plus the same defect in `sync-manager.js`'s skeleton stale-cleanup (previously an inert no-op for the same reason); verified live, post-fix, on the real page that reported it (add + delete, no reload, twice). Not a regression from this session — all four bugs' code predates today's earlier IndexedDB-migration work (bug 1 by ~6 weeks pre-sync entirely; bugs 2/4 untouched since 2026-03-17/2026-03-09 respectively). Task file has a full 8-combination test matrix (split mode × multi-tenant × sync, each × 9 lifecycle stages) — 2 of 8 combinations tested; bug 4's fix is confirmed independent of all three matrix axes so doesn't need re-testing across the grid.
