...
**STATUS: IMPLEMENTED, with a post-ship fix.** All files changed as planned, `npm run build` passes. One design deviation from the original plan (see below). Live-render verification (the Testing Checklist) still pending.
...
- [ ] On the transportny theme specifically: confirm `npmrdsMeasureMenu`/`npmrdsQuickControls` (registered under `"AVL Graph"` in `theme.js`/`themev2.js`) still appear on AVL Graph sections — this is the exact regression the Design Deviation avoided, worth a direct spot-check.
