...
- [x] **Bug 18 (PARTIALLY FIXED, 2026-09-14) — a regular browser tab shows stale `page_2` content on `shaun-test-app` that an incognito window does not; separately, the same-shaped mechanism destroyed a real day of editor work on production `mitigat-ny-prod`/`county_template` on 2026-09-11.** Root-caused and live-reproduced (see "Live reproduction and fix" subsection above): `sectionArray.jsx`'s page-structure room seeds itself once, synchronously, at mount (`value` deliberately excluded from the effect's deps) — if local-first sync's bootstrap is still resolving at that instant (an SPA client-side nav reusing stale loader data, or a race with a concurrent bootstrap), the room locks onto a stale/empty snapshot forever, even though the page's `value` correctly updates moments later for **rendering only** — every later structural edit from that mount then silently overwrites the server's real content with (stale room base) + (this one new op). Fixed via `room.reseedIfEmpty(value)`, called with the current render's `value` immediately before `save()`/`remove()`/`moveItem()` apply their own op, giving the stale seed a second, later chance to correct itself. Verified live: 2/2 pre-fix trials of the SPA-nav-race reproduced the loss; 2/2 post-fix trials preserved all content. Separately, `bumpLastRevision` is implemented for the theorized gap #2 (`catchUp()`'s unscoped-watermark decoupling) but a same-day live dump initially claimed as confirming it (2026-09-14) turned out to be a misread — it compared different apps' watermarks sharing one browser's `dms-sync` IndexedDB, not one app's decoupled scopes (see "Correction" under "Gap #2 confirmed live in the wild, and fixed" below). Gap #2 itself is **still unconfirmed either way**. A separate, unrelated symptom that same session (stale pages reappearing on `shaun-test-app` nav) turned out to be a stale local mirror from an out-of-band data change, not a sync-code bug — resolved via `resetAndRebootstrap()`, see "Separate incident" subsection below. **Still open**: a continuously-open tab that never navigates/reloads at all before its first edit is not fixed by the gap #1 fix (a separate, not-yet-root-caused gap in why the live WS broadcast didn't update that tab's `value` either); gap #2 needs an actual live reconnect trial (not another IndexedDB dump comparison) to confirm or refute. Do not treat Bug 18 as fully closed.
