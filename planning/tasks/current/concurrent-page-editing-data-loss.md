...
- [ ] **Bug 18 (OPEN, now with a production incident) — a regular browser tab shows stale `page_2` content on `shaun-test-app` that an incognito window does not.** Confirmed unrelated to the `local-first-sync-updates` × `master` merge it was found during (`packages/dms/src/sync/` is byte-identical between the two branches). Not yet reproduced under instrumentation — likely mechanism is a gap in the regular tab's WS live-update or reconnect-catchup path leaving its local IndexedDB copy stale, per `sync.isLocal()`'s local-first read routing, but this is inferred from `sync/CLAUDE.md`'s documented architecture, not confirmed live. **2026-09-11: matching mechanism destroyed a real day of editor work on production `mitigat-ny-prod`/`county_template`** — see "Real-world corroboration" subsection above for the full incident. Raises priority.
