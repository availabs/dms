...
## Current state (2026-07-16, ROUND 54 COMPLETE: the pre-2017-only report-level refusal (item 6's round-53 regression finding) is RESTORED, live-verified against the exact 4 reports it used to block, and census-mirrored — see "Round 54" below. Round 53's 9-item triage + item 0 (stray `reports_snap_2` rows) remain DONE per that round's entries; the other 7 non-priority-1 triage items (BarGraph tooltip, graph title default, GridGraph missing-data color, TMC meta join swap, Info Box travel-time formatter, epoch x-axis tick format, legend/flex width-squeeze) are still open, see the priority list at the end of the Round 53 section.)

## Round 54 (2026-07-16) — restore the pre-2017-only report-level refusal (regression fix)
...
- **Full census rerun** (869 reports, 0 errors): `pre_2017_only_count: 133 (15.3%)` — matches round
  ...
  awaiting a user decision on whether to delete it like the original 4.

**Not done**: no proactive resweep of the rest of the corpus, no decision yet on report 7's page.
Everything else in Round 53's priority list (BarGraph tooltip, graph title default, GridGraph
missing-data color, TMC meta join swap, Info Box travel-time formatter, epoch x-axis tick format,
legend/flex width-squeeze) is still open and unchanged by this round.

---

## Round 53 triage (2026-07-16, IN PROGRESS — user gave a 9-item punch list, this section tracks status per item so a context-cut resume doesn't re-derive root causes)
...
3. **BarGraph tooltip customName fix** (item 8, tooltip half) — small, isolated platform fix (`BarGraph.jsx`), likely fixes the raw-SQL tooltip corpus-wide in one place.
